...
def update_metadata(timestamp, total_users, active_users, active_users_ratio, metadata_file_path="users.meta.csv"):
    if not os.path.exists(metadata_file_path):
        df = pd.DataFrame(columns=['Timestamp', 'TotalUsers', 'ActiveUsers', 'ActiveUsersRatio'])
    else:
        df = pd.read_csv(metadata_file_path)
    df = df.append({'Timestamp': timestamp, 'TotalUsers': total_users, 'ActiveUsers': active_users, 'ActiveUsersRatio': active_users_ratio}, ignore_index=True)
    ...
    try:
        ...
        try:
            ...
            table_data = [
                ...
                ["Ratio of active users to total users", Fore.MAGENTA + f"{active_users_ratio:.2%}"],
            ]
